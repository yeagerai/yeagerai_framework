...
class AbstractComponent(abc.ABC):
    to_implement_methods: typing.List[str] = ["__init__", "transform"]

    @abc.abstractmethod
    async def transform(self, args: typing.Any, callbacks: typing.Any) -> typing.Any:
        """Transform functionality of the button. This is where the actual button behaviour goes."""
        raise NotImplementedError("Please Implement this method")

    async def fit(self, args: typing.Any, callback: typing.Any) -> None:
        """Fit functionality of the button. \
        Reimplement only in case the button needs to fit parameters."""
        raise NotImplementedError(
            "Trying to call fit without implementing it. Please implement it first."
        )
    ...
    @classmethod
    def check_init_parameters_reserved_words(cls) -> None:
        signature = inspect.signature(cls)
        parameters = signature.parameters

        for parameter in parameters.values():
            if parameter.name in [_CLASSNAME_TAG, _VALUE_TAG]:
                raise ValueError(
                    f"{parameter.name} is a reserved word, please rename the parameter"
                )
        return
    ...
    def test_cls(cls) -> None:
        if "__init__" in cls.to_implement_methods:
            ...
            cls.check_init_parameters_reserved_words()
